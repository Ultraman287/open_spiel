...
#ifndef OPEN_SPIEL_GAMES_MINICHESS_H_
#define OPEN_SPIEL_GAMES_MINICHESS_H_
...
namespace open_spiel
{
  namespace minichess
  {
    ...
    // https://math.stackexchange.com/questions/194008/how-many-turns-can-a-minichess-game-take-at-maximum
    ...
    inline const std::vector<int> &ObservationTensorShape()
    {
      static std::vector<int> shape{
          11 /* piece types * colours + empty */ + 1 /* repetition count */ +
    ...
    class MinichessState : public State
    {
    public:
      // Constructs a minichess state at the standard start position.
      MinichessState(std::shared_ptr<const Game> game);

      // Constructs a minichess state at the given position in Forsyth-Edwards Notation.
      // https://en.wikipedia.org/wiki/Forsyth%E2%80%93Edwards_Notation
      MinichessState(std::shared_ptr<const Game> game, const std::string &fen);
      MinichessState(const MinichessState &) = default;

      MinichessState &operator=(const MinichessState &) = default;
      ...
      // Current board.
      MinichessBoard &Board() { return current_board_; }
      const MinichessBoard &Board() const { return current_board_; }
      int BoardSize() const { return current_board_.BoardSize(); }

      // Starting board.
      MinichessBoard &StartBoard() { return start_board_; }
      const MinichessBoard &StartBoard() const { return start_board_; }
      ...
      // algebraic notation (using MinichessBoard::ParseMove), or kInvalidAction if
      // the parsing fails.
    ...
    private:
      ...
      MinichessBoard start_board_;
      // We store the current board position as an optimization.
      MinichessBoard current_board_;
    ...
    // Game object.
    class MinichessGame : public Game
    {
    public:
      explicit MinichessGame(const GameParameters &params);
      int NumDistinctActions() const override
      {
        return minichess::NumDistinctActions();
      }
      std::unique_ptr<State> NewInitialState(
          const std::string &fen) const override
      {
        return absl::make_unique<MinichessState>(shared_from_this(), fen);
      }
      std::unique_ptr<State> NewInitialState() const override
      {
        return absl::make_unique<MinichessState>(shared_from_this());
      }
      int NumPlayers() const override { return minichess::NumPlayers(); }
      ...
      {
        return minichess::ObservationTensorShape();
      }
      int MaxGameLength() const override { return minichess::MaxGameLength(); }
    };

  } // namespace minichess
} // namespace open_spiel

#endif // OPEN_SPIEL_GAMES_MINICHESS_H_
