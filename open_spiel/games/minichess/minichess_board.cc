...
namespace open_spiel
{
  namespace minichess
  {
    ...
    bool IsLongDiagonal(const minichess::Square &from_sq, const minichess::Square &to_sq,
                        int board_size)
    {
      if (from_sq == to_sq)
      {
    ...
    std::string Move::ToSAN(const MinichessBoard &board) const
    {
      ...
      else if (disambiguation_required)
      {
        if (file_unique)
        {
          ...
          // use file. [FIDE Laws of Minichess (2018): C.10.3].
          file_required = true;
        }
        else if (rank_unique)
        {
    ...
    MinichessBoard::MinichessBoard(int board_size, bool king_in_check_allowed,
    ...
    /*static*/ absl::optional<MinichessBoard> MinichessBoard::BoardFromFEN(
        ...
        bool king_in_check_allowed, bool allow_pass_move)
    {
       ...
       */
      MinichessBoard board(board_size, king_in_check_allowed, allow_pass_move);
      ...
      // minichess960 there must be a rook on either side of the king, but all 3 can
      ...
      // standard minichess), we assume it's standard minichess, and use that as the rook,
      // even if there are multiple rooks.
      // Note that this can create ambiguous minichess960 positions, but we don't have
    ...
    Square MinichessBoard::find(const Piece &piece) const
    {
    ...
    void MinichessBoard::GenerateLegalMoves(const MoveYieldFn &yield,
                                            Color color) const
    {
    ...
    void MinichessBoard::GeneratePseudoLegalMoves(
...
#undef YIELD
    ...
    void MinichessBoard::GenerateLegalPawnCaptures(const MoveYieldFn &yield,
                                                   Color color) const
    {
      // We do not need to filter moves that would result for King to move / stay
      // in check, so we can yield all pseudo legal moves
    ...
    void MinichessBoard::GeneratePseudoLegalPawnCaptures(
        const MoveYieldFn &yield, Color color,
        PseudoLegalMoveSettings settings) const
    {
      bool generating = true;
...
#undef YIELD
    ...
    bool MinichessBoard::IsBreachingMove(Move tested_move) const
    {
      if (tested_move == kPassMove)
        return false;
    ...
    void MinichessBoard::BreachingMoveToCaptureMove(Move *move) const
    {
    ...
    bool MinichessBoard::HasSufficientMaterial() const
    {
    ...
    absl::optional<Move> MinichessBoard::ParseMove(const std::string &move) const
    {
    ...
    absl::optional<Move> MinichessBoard::ParseSANMove(
        const std::string &move_str) const
    {
      ...
      // IMPLEMENTATION POTENTIAL UPDATE
    ...
    absl::optional<Move> MinichessBoard::ParseLANMove(const std::string &move) const
    {
      if (move.empty())
      {
        return absl::nullopt;
    ...
    void MinichessBoard::ApplyMove(const Move &move)
    {
      ...
      if (move == kPassMove)
      {
        ...
        SetEpSquare(minichess::kInvalidSquare);
        return;
      }
      ...
      // We have to do it in this order because in Minichess960 the king can castle
      ...
      if (move.is_castling)
      {
        ...
        // even in minichess960 (destination squares are same as in normal minichess).
    ...
    bool MinichessBoard::TestApplyMove(const Move &move)
    {
    ...
    bool MinichessBoard::UnderAttack(const Square &sq, Color our_color) const
    {
    ...
    std::string MinichessBoard::DebugString() const
    {
      std::string s;
    ...
    void MinichessBoard::GenerateKingDestinations_(Square sq, Color color,
    ...
    bool MinichessBoard::CanCastleBetween(
        ...
        PseudoLegalMoveSettings settings) const
    {
      ...
      for (int x = x_start; x <= x_end; ++x)
      {
        ...
        if (x_in_between && IsFriendly(test_square, our_color))
          return false;
      }
      return true;
    }
    ...
    template <typename YieldFn>
    void MinichessBoard::GenerateCastlingDestinations_(Square sq, Color color,
                                                       ...
                                                       const YieldFn &yield) const
    {
      ...
      //
      // We use the generalized definition of castling from Minichess960, instead of
      // hard-coding starting squares.
      // By Minichess960 rules, the king and rook end up in the same positions as in
      // standard minichess, but they can start from any squares.
      //
      ...
      // castling is not defined for other minichessboards than the standard one
      if (board_size_ != 8)
      {
        return;
      }
    ...
    template <typename YieldFn>
    void MinichessBoard::GenerateQueenDestinations_(Square sq, Color color,
                                                    PseudoLegalMoveSettings settings,
                                                    const YieldFn &yield) const
    {
    ...
    template <typename YieldFn>
    void MinichessBoard::GenerateRookDestinations_(Square sq, Color color,
                                                   PseudoLegalMoveSettings settings,
                                                   const YieldFn &yield) const
    {
    ...
    template <typename YieldFn>
    void MinichessBoard::GenerateBishopDestinations_(Square sq, Color color,
                                                     PseudoLegalMoveSettings settings,
                                                     const YieldFn &yield) const
    {
    ...
    template <typename YieldFn>
    void MinichessBoard::GenerateKnightDestinations_(Square sq, Color color,
                                                     const YieldFn &yield) const
    {
    ...
    template <typename YieldFn>
    void MinichessBoard::GeneratePawnDestinations_(Square sq, Color color,
                                                   PseudoLegalMoveSettings settings,
                                                   const YieldFn &yield) const
    {
    ...
    template <typename YieldFn>
    void MinichessBoard::GeneratePawnCaptureDestinations_(
    ...
    template <typename YieldFn>
    void MinichessBoard::GenerateRayDestinations_(Square sq, Color color,
                                                  PseudoLegalMoveSettings settings,
                                                  Offset offset_step,
                                                  const YieldFn &yield) const
    {
    ...
    std::string MinichessBoard::ToUnicodeString() const
    {
      std::string out = "\n";
    ...
    std::string MinichessBoard::ToFEN() const
    {
    ...
    // Used in Dark Minichess (see games/dark_minichess.{h,cc})
    std::string MinichessBoard::ToDarkFEN(const ObservationTable &observability_table,
                                          Color color) const
    {
      std::string fen;
      ...
      for (int8_t rank = board_size_ - 1; rank >= 0; --rank)
      {
        ...
        for (int8_t file = 0; file < board_size_; ++file)
        {
          size_t index = SquareToIndex_(minichess::Square{file, rank});
          ...
          {
            const Piece &piece = at(minichess::Square{file, rank});
            if (piece == minichess::kEmptyPiece)
            {
              ++num_empty;
            }
            else
      ...
      // 2. color to play.
      absl::StrAppend(&fen, " ", ToPlay() == minichess::Color::kWhite ? "w" : "b");

      // 3. by castling rights.
      absl::StrAppend(&fen, " ");
      std::string castling_rights;
      if (color == minichess::Color::kWhite)
      {
        if (CastlingRight(minichess::Color::kWhite, minichess::CastlingDirection::kRight))
        {
          castling_rights.push_back('K');
        }
        if (CastlingRight(minichess::Color::kWhite, minichess::CastlingDirection::kLeft))
        {
          castling_rights.push_back('Q');
        }
      }
      ...
      {
        if (CastlingRight(minichess::Color::kBlack, minichess::CastlingDirection::kRight))
        {
          castling_rights.push_back('k');
        }
        if (CastlingRight(minichess::Color::kBlack, minichess::CastlingDirection::kLeft))
        {
          castling_rights.push_back('q');
        }
      }
    ...
    void MinichessBoard::set_square(Square sq, Piece piece)
    {
    ...
    absl::optional<Square> MinichessBoard::MaybeCastlingRookSquare(
    ...
    void MinichessBoard::SetCastlingRight(Color side, CastlingDirection direction,
    ...
    Square MinichessBoard::FindRookForCastling(Color color,
                                               CastlingDirection dir) const
    {
      ...
      else
      {
        ...
        // Failing here means the FEN is either from minichess960 or malformed (the FEN
        ...
        // partial support for minichess960, but not for loading a mid-game minichess960
        ...
        // with then). Solving this will require implementing a minichess960-specific
        // FEN format.
    ...
    void MinichessBoard::SetToPlay(Color c)
    {
    ...
    void MinichessBoard::SetIrreversibleMoveCounter(int c)
    {
      irreversible_move_counter_ = c;
    }

    void MinichessBoard::SetMovenumber(int move_number) { move_number_ = move_number; }

    void MinichessBoard::SetEpSquare(Square sq)
    {
    ...
    MinichessBoard MakeDefaultBoard()
    {
      auto maybe_board = MinichessBoard::BoardFromFEN(kDefaultStandardFEN);
    ...
    {
      if (board_size == 8)
        return minichess::kDefaultStandardFEN;
      else if (board_size == 4)
        return minichess::kDefaultSmallFEN;
      else
        SpielFatalError(
            "Only board sizes 4 and 8 have their default minichessboards. "
