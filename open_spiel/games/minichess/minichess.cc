...
namespace open_spiel
{
  namespace minichess
  {
    namespace
    {
      ...
      {
        return std::shared_ptr<const Game>(new MinichessGame(params));
      }
    ...
    MinichessState::MinichessState(std::shared_ptr<const Game> game)
        : State(game),
    ...
    MinichessState::MinichessState(std::shared_ptr<const Game> game, const std::string &fen)
        : State(game)
    {
      auto maybe_board = MinichessBoard::BoardFromFEN(fen);
    ...
    Action MinichessState::ParseMoveToAction(const std::string &move_str) const
    {
    ...
    void MinichessState::DoApplyAction(Action action)
    {
    ...
    void MinichessState::MaybeGenerateLegalActions() const
    {
    ...
    std::vector<Action> MinichessState::LegalActions() const
    {
      ...
      if (is_under_promotion)
      {
        ...
        int direction_index;
        {
          auto itr = absl::c_find_if(
              ...
              [offset](Offset o)
              { return o.x_offset == offset.x_offset; });
    ...
    Move ActionToMove(const Action &action, const MinichessBoard &board)
    {
      SPIEL_CHECK_GE(action, 0);
      SPIEL_CHECK_LT(action, NumDistinctActions());

      // Some minichess variants (e.g. RBC) allow pass moves.
      ...
      // by 2 spaces.
      // TODO(b/149092677): Minichess no longer supports minichess960. Distinguish between
      // left/right castle.
    ...
    std::string MinichessState::ActionToString(Player player, Action action) const
    {
      Move move = ActionToMove(action, Board());
      return move.ToSAN(Board());
    }

    std::string MinichessState::ToString() const { return Board().ToFEN(); }

    std::vector<double> MinichessState::Returns() const
    {
    ...
    std::string MinichessState::InformationStateString(Player player) const
    {
    ...
    std::string MinichessState::ObservationString(Player player) const
    {
      SPIEL_CHECK_GE(player, 0);
      SPIEL_CHECK_LT(player, num_players_);
      return ToString();
    ...
    void MinichessState::ObservationTensor(Player player,
    ...
    std::unique_ptr<State> MinichessState::Clone() const
    {
      return std::unique_ptr<State>(new MinichessState(*this));
    }

    void MinichessState::UndoAction(Player player, Action action)
    {
    ...
    bool MinichessState::IsRepetitionDraw() const
    {
      const auto entry = repetitions_.find(Board().HashValue());
      SPIEL_CHECK_FALSE(entry == repetitions_.end());
    ...
    absl::optional<std::vector<double>> MinichessState::MaybeFinalReturns() const
    {
    ...
    MinichessGame::MinichessGame(const GameParameters &params) : Game(kGameType, params) {}
