...
#ifndef OPEN_SPIEL_GAMES_IMPL_MINICHESS_MINICHESS_BOARD_H_
#define OPEN_SPIEL_GAMES_IMPL_MINICHESS_MINICHESS_BOARD_H_
...
#include "open_spiel/games/minichess/minichess_common.h"
...
namespace open_spiel
{
  namespace minichess
  {
    ...
    using minichess_common::kInvalidSquare; // NOLINT
    using minichess_common::Offset;
    using minichess_common::Square;
    using minichess_common::SquareToString; // NOLINT

    template <std::size_t... Dims>
    using ZobristTableU64 = minichess_common::ZobristTable<uint64_t, Dims...>;
    ...
    // Forward declare MinichessBoard here because it's needed in Move::ToSAN.
    ...
    struct Move
    {
      ...
      // We have to record castling here, because in Minichess960 we may not be able to
      ...
      // notation (PGN). It is a minichess move notation that is designed to be
      ...
      std::string ToSAN(const MinichessBoard &board) const;
    ...
    // Some minichess variants (RBC) allow a "pass" action/move
    inline constexpr open_spiel::Action kPassAction = 0;
    inline const minichess::Move kPassMove =
    ...
    {
    public:
      MinichessBoard(int board_size = kDefaultBoardSize,
                     bool king_in_check_allowed = false,
                     bool allow_pass_move = false);

      // Constructs a minichess board at the given position in Forsyth-Edwards Notation.
      // https://en.wikipedia.org/wiki/Forsyth%E2%80%93Edwards_Notation
      static absl::optional<MinichessBoard> BoardFromFEN(
      ...
      // However, detecting all such positions will require solving minichess... so
      ...
      // https://en.wikipedia.org/wiki/Algebraic_notation_(minichess).
      ...
      // form used by minichess engine text protocols that are of interest to us.
      // Returns absl::nullopt on failure.
      ...
      // Constructs a string describing the minichess board position in Forsyth-Edwards
      // Notation. https://en.wikipedia.org/wiki/Forsyth%E2%80%93Edwards_Notation
      std::string ToFEN() const;

      /* Constructs a string describing the dark minichess board position in a notation
    ...
    private:
      ...
      // minichess is a "half move" by white followed by a "half move" by black).
    ...
    inline std::ostream &operator<<(std::ostream &stream, const MinichessBoard &board)
    {
...
#endif // OPEN_SPIEL_GAMES_IMPL_MINICHESS_MINICHESS_BOARD_H_
